...
export function SelectAccountAddingFlow({ flow, onSelect, onNext }: Props): JSX.Element {
    ...
    const [selectedDerivedKey, setDerivedKey] = React.useState<OptionType | null>(
        derivedKeysOptions[0]
    )
    ...
    React.useEffect(() => {
        const derivedKey =
            selectedDerivedKey && accountability.derivedKeys[selectedDerivedKey?.value]
        if (derivedKey != null) {
