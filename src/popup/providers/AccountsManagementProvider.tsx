...
import { AccountToCreate, KeyToDerive, MasterKeyToCreate } from '@shared/approvalApi'
...
export function AccountsManagementProvider({ children }: Props): JSX.Element {
	...
	// Derived keys
	const derivedKeys = React.useMemo(() => Object.values({ ...rpcState.state?.storedKeys }).filter(
	...
	const derivedKeyRelatedAccounts = React.useMemo(() => {
		return currentDerivedKey ? (rpcState.state?.accountEntries[currentDerivedKey.publicKey] || []) : []
	...
	const accounts = React.useMemo(() => {
		const derivedKeysPubKeys = Object.values({
			...rpcState.state?.storedKeys,
		}).filter((key) => (key.masterKey === rpcState.state?.selectedMasterKey)).map((key) => key.publicKey)
		...
		return availableAccounts.filter(
			(account) => account.tonWallet !== undefined
	...
	const accountAddress = React.useMemo(
		() => rpcState.state?.selectedAccount?.tonWallet.address,
		[rpcState.state?.selectedAccount]
	)

	const accountName = React.useMemo(
		() => rpcState.state?.selectedAccount?.name,
		[rpcState.state?.selectedAccount]
	)

	const nextAccountId = React.useMemo(() => {
		const ids = Object.values({ ...rpcState.state?.storedKeys }).map(({ accountId }) => accountId)
